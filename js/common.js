...
links.forEach(a=>{ const file = a.getAttribute('href'); if (file === here) a.classList.add('active'); });
})();


// Sous‑nav : drag‑to‑scroll (desktop)
(() => {
...
const onDown = (e) => { isDown = true; track.classList.add('grabbing'); startX = getX(e); startScroll = track.scrollLeft; };
const onMove = (e) => { if (!isDown) return; e.preventDefault(); const x = getX(e); track.scrollLeft = startScroll - (x - startX); };
const onUp = () => { isDown = false; track.classList.remove('grabbing'); };
track.addEventListener('mousedown', onDown); track.addEventListener('mousemove', onMove); window.addEventListener('mouseup', onUp);
...
track.addEventListener('keydown', (e) => { if (e.key === 'ArrowRight') track.scrollBy({ left: 120, behavior: 'smooth' }); if (e.key === 'ArrowLeft') track.scrollBy({ left:-120, behavior: 'smooth' }); });
})();
...
// IMPORTANT: cible le panneau animé
const panel = menu.classList.contains('navmenu-panel') ? menu : null;
